...
//
layout(std430, binding = 3) buffer WeightGradientBuffer {
    float weight_gradients[];
};

layout(std430, binding = 4) buffer BiasGradientBuffer {
...
//
layout(std140, binding = 5) uniform LayerMeta {
...
//
float compute_tanh_derivative(float activated_value) {
...
//
float compute_neuron_delta(uint vector_index, uint neuron_index) {
    uint offset = vector_index * output_size + neuron_index;
    float activation = activations[offset];
    float error = errors[offset];
    float derivative = compute_tanh_derivative(activation);
...
void main() {
    ...
    float delta = compute_neuron_delta(vector_index, neuron_index);
